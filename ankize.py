...
with open(ITEMS) as f:
    ...
    for elem in d:
        ...
        if elem['Video'] != '':
          try:
              os.mkdir(os.path.join(DEST, str(SECTION_ID)))
          except OSError:
              pass
          videoName = elem['Video'][-9:]+'.webm'
          shutil.move(os.path.join(BUILD, str(SECTION_ID), videoName), os.path.join(DEST, 'videos', videoName))
          # Doesn't work
